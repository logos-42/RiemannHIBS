...
## 缺什么：推导式定位 (2026-09-01, leo 推导)

**已证清单按逻辑类型分类 —— 三类都有, 唯独缺第四种「排除」:**

| 类型 | 内容 | 能推出什么 |
|---|---|---|
| 恒等式/等价 (⟺) | 坐标字典、反射配对、自配对代数、反演不动圆 | **不产生新信息** |
| 发散/丰度 (→∞) | 调和能量发散、Hardy 传输、无限零点 | 只给「存在」, **不给「排除」** |
| 临界性 (1/2 特殊) | 能量收敛阈值、唯一不动圆 | 只给「位置」 |

**RH = ¬∃ 零点满足 ρ≠√e — 「排除」型.** 清单里没有第四种类型, 这就是缺口.

**关键推导 (为什么频率+收敛在临界线未定义好):** 分部求和, 对 0<σ<1:
```
Σ_{n≤N} n^{-s} = ζ(s) + N^{1-s}/(1-s) + O(N^{-σ})
```
σ<1 时部分和**不收敛**: 绕 ζ(s) 旋转、半径 N^{1-σ}/|1-s| 的**外旋螺旋** (σ=1/2 时 ~N^{1/2}/t→∞). 推论: ①「S_N→0」在临界线**恒假** (R5c 数值撞见: γ₁ 处 |S_N| 0.25→1.42 而 ζ=0); ② ζ(s)=0 真身 = **螺旋中心是原点**, 不是部分和趋于 0; ③ 从部分和到中心 = 解析延拓/重求和 (η 重整、Euler–Maclaurin、近似函数方程 Riemann–Siegel), **每一步都要余项估计 — 这就是缺的桥**.

**RS 截断警告:** RS 主和项数 N=⌊√(t/2π)⌋ ≪ 频率折返点 n*≈t/π (t=1000: 12.6 vs 318) — **折返点落在被丢弃的发散尾部** (部分和图像已失效的区域). `fold_point_exists` 目前是描述性对象, **与零点的联系未建立**.

**坐标对应是布尔代数同构 (可证):** Φ:s↦e^s 双射, 翻译全是 ⟺, Γ⊢φ ⟺ T(Γ)⊢T(φ). **坐标系统既不帮忙也不碍事, 没有制造内容的能力.** 真实贡献只有两条: 把目标说干净 (RH = 环内无非自反射轨道); 提出并回答经典提不出的问题 (为什么 √e — 因为 log√e=1/2).

**唯一 σ-不对称量 = 缺口入口:** 函数方程 Λ(s)=Λ(1−s) 的 Γ 侧不对称, 由 Stirling:
```
|Γ(s/2)| / |Γ((1−s)/2)| ~ (t/2)^{σ−1/2} = (t/2)^{log(ρ/√e)},  ρ=|w|
```
反射把 ρ 配到 e/ρ, 两侧「质量」恰在 ρ=√e 时等于 1. 这是全装置**唯一的 σ-不对称量** — 坐标内任何排除性论证都得经过它 — 而它是 Stirling 级输入 (P1–P3 诚实边界, **未形式化**).
...
- **B. 真正的墙 (σ>1/2 上 ζ≠0 = RH 本身) — 2026-09-01 侦察 + 骨架化**: mathlib 家底: Phragmén–Lindelöf ✅ (唯一性形态, PhragmenLindelof.lean) + Borel–Carathéodory ✅ + 最大模 ✅; **❌ 零密度 / AFE / Riemann–Siegel / Lindelöf 估计 / 无零点区域** — 真墙在 mathlib 之外. 已做: ①**3-4-1 代数核心砖 (真定理, Abundance §22)**: `three_four_one_nonneg` (3+4cosθ+cos2θ=2(cosθ+1)²≥0) + `mertens_coeff_re_nonneg` (Re(3+4(n+1)^{−it}+(n+1)^{−2it})≥0) — 无零点区域论证 (ζ(1+it)≠0) 的代数构建块; ②**`ZeroFreeHalfPlane` 结构体** — 依赖链钉形状: 已证 (反射/σ≥2 零域/3-4-1) + 外部输入 (条带增长上界 + 零密度论证, 经典文献供给). **B 的供给清单**: Hadamard–de la Vallée Poussin 无零点区域 (σ≥1−c/log t) / Ingham–Selberg 零密度 / Riemann–Siegel AFE 余项 / Lindelöf 函数 L(σ).
...
- **A 闸门 (Abel 分部求和) 已落码 (2026-09-01, Abundance §24, 无 sorry 编译通过)**: abel_sum_by_parts (Abel 恒等式, 纯代数) + alternating_abel_norm_le_one (交错部分和模≤1) + **alternating_sum_bound** (Dirichlet 主估计 ‖Σ(−1)^n b_n‖ ≤ ‖b_M‖+Σ‖b_n−b_{n+1}‖, 与 M 无关 ⟹ Cauchy 尾估计). **[A] 是 Hadamard 链(条带 ζ 上界) 与 §21 η 桥的共同钥匙** — 无它任何排除性估计(零密度/无零点区域/Lindelöf)进不了场. **闸门≠终点**: 走完 Hadamard 链只复现 Hadamard 1893(非平凡零点无限多), 不证 RH. 剩余为独立工作量: [B] Γ 上界(**初等推导已给出, 未落码**: |Γ(σ+it)| ≤ Γ(σ+n)/|t|^n, n=⌈|t|⌉, 配实轴粗上界 Γ(x+1) ≤ (2x)^{x+1}+2(2x)^x ⟹ |Γ| ≤ 2^{|t|}|t|^σ = e^{O(|t|)}, **无 Stirling/无 Laplace/无 Gaussian**, 远强于所需的 e^{O(|s|log|s|)}) + [C][E] 组合与 Borel–Carathéodory 应用 + 零密度输入
- **bounded_variation 已从输入变成定理 (2026-09-01, Abundance §25, 无 sorry 编译通过)**: `eta_bounded_variation` (Σ‖(n+1)^{−s}−(n+2)^{−s}‖ 可和, σ>0, 经 `complex_exp_sub_one_norm_le_two`(全局 |1−e^z|≤2|z|e^{|z|}) + `eta_variation_step`(精确步) + p 级数) + `eta_tends_to_zero` (b_n→0) + **`eta_partial_sums_converge`** (η 部分和 ∃ 极限 = η 条件收敛正确形态, 非 Summable 因 ℂ 上 Summable⟺绝对可和; Cauchy = alternating_sum_bound 尾 + 变差可和 + 项→0 + ℂ 完备). ⟹ §21 `EtaCriticalStripBridge.conditional_convergence_strip` 与 §24 `AbelGate.bounded_variation/tends_to_zero` 两个「外部输入」现已**可证**. **诚实: bookkeeping 而已 — 让对齐语言在临界带良定, 不携带任何排除信息, 不缩短 RH**; Hadamard 链剩余 [B]/[C][E]/零密度仍是独立工作量 (走到底只复现 Hadamard 1893)
- **组合 B: 内禀排除判据已落码 (2026-09-01, Abundance §28, 无 sorry 编译通过)**: `zetaChi` + 严格 ⟺ `zeta_zero_iff_pair_both_zero` (零点 ⟺ 反射对同时消失, 组合 §23 zeta_mod_balance) + `zeta_norm_ne_zero_iff_pair` (非零 ⟺ 反射对同时非零, 组合 §16 zeta_zero_iff_one_sub) + 结构体 `GammaImbalanceReduction` (已证 5 字段 + chi_asymptotic Stirling 输入 → zeroFreeHalfPlane). **反思: 两条严格 ⟺ 是「反射配对」的零/非零两版, 不携带排除新强度 (信息守恒的必然); 真价值 = 钉死「排除 = 反射对整体非零」并把唯一定量缺口精确到 χ 渐近 (等价 Lindelöf)**
- **组合 B 全局 ⟺ 已闭环 (2026-09-01, Abundance §30, 无 sorry 编译通过)**: **`riemannHypothesis_iff_zeroFreeHalfPlane`** 严格成立 (RH ⟺ σ>1/2 无零点). 方向 1 (RH→排除) 用 mathlib `riemannZeta_ne_zero_of_one_le_re` (σ≥1, 含 s=1); 方向 2 (排除→RH) 用 sin 版函数方程 `zeta_eq_sin_mul_zeta_one_sub` + `zeta_ne_zero_of_nonpos_re_of_nontrivial` (σ≤0 排除) + 反射配对 + 排除本身. **关键结论: 「排除 ⟺ RH」的证明全程无 Stirling, 只需函数方程 + sin/Γ 零点分类 + σ≥1 无零点 (PNT 前提, mathlib 已有); Stirling 只在「证明排除本身」(= Lindelöf) 时才需要. 这精确界定了「缺什么」: 不是缺排除的等价刻画 (已钉死), 是缺那条等价于 Lindelöf 的定量估计**
...
chi_mod_decomp:   |χ(s)| = 2·(2π)^{−σ}·|Γ(s)|·|cos(πs/2)|
其中 χ(s) = 2·(2π)^{−s}·Γ(s)·cos(πs/2) 是唯一携带 σ-幂的传输因子.
```
**数学含义**: |χ(σ+it)| ~ C_σ·t^{σ−1/2} (Stirling), 故模平衡给 σ>1/2 侧 ζ 模 = t^{σ−1/2} 增长因子 × σ<1/2 侧 ζ 模. 这是「离圆零点排除」的候选载体: σ≠1/2 ⟹ |χ| 的 t-增长非平凡 ⟹ 反射两侧模失衡. **但零点处 0=0 退化** — 排除需要「接近零点的模比」或「零点重数」论证.

**硬缺口 (关键张力)**: 排除需要 **t-方向** 的模估计 (Stirling 级); 已有的 Γ 递推下界 `gamma_ge_pow_mul_self` (Γ(x+n) ≥ xⁿ·Γ(x)) 是 **x-方向 (实轴)** 的 — 两个方向正交. **下一步 = 复数递推下界** |Γ(x+n+iy)| ≥ |y|^n·|Γ(x+iy)| (每个因子 |x+k+iy| = √((x+k)²+y²) ≥ |y|), 把 x-方向推广到 t-方向, 才能不经 Stirling 进入模平衡. 这是「后续怎么做」的具体答案.
...
- R5 对齐密度猜想 (Abundance §17 + experiments/fig15, 本日提交):
  ...
  F(T) = (T/2π)(log(T/2π) − 1), 无需 Stirling; 数值 (fig15): **N(T)/F(T) → 1**
  (T=100..3200: 1.0310 → 1.0003) + δ̄=1.0000 (频率均匀性) + 反演对分解数值对照
  (平凡零点 −2k 的反演对 1+2k 处 ζ≠0 — 排斥因子破坏对结构; 临界带零点成对
  |ζ(1−s)|≈0 ✓) — 对分解只属临界带; **R5c 诚实发现**: 临界带内 Dirichlet
  部分和 |S_N| 不趋于 0 (γ₁ 处随 N 增长 0.25→1.42, 尾项 N^{-1/2}→0) —
  对齐 (零点) 是解析延拓的零点非级数部分和零点 ⟹ §10 对齐条件的 Re>1 版本
  不能直接搬进临界带; 猜想严格化需等分布/加权 Weyl 准则 (mathlib 无)

- R6 频率结构 (Abundance §18, 无 sorry, 本日提交) — 只看频率, 不看能量:
  freq_gap_tendsto_zero (频率间距 Δfreq = log((n+2)/(n+1)) → 0, log 连续 +
  (n+2)/(n+1)→1) + freq_gap_antitone (间距单调递减: (n+3)/(n+2) ≤ (n+2)/(n+1)
  经 log_le_log + 初等代数) + **phase_freeze** (相位冻结: ∀ε>0, ∃N, t·Δfreq < ε
  — 高频项相位差任意小, 同向叠加) + **fold_point_exists** (相位折返点:
  t·log 2 > π ⟹ ∃n, π−t·Δfreq n ≤ 0 < π−t·Δfreq(n+1) — η 交错项相位差
  Δθ = π−t·Δfreq 单调递增从负到正 ⟹ 折返点 n* ≈ t/π 存在, 经 Nat.find
  最小正点) — **零点高度 t 在频率域的回声**: 相位旋转在 n* ≈ t/π 处折返,
  这是坐标系独有的频率结构 (经典框架无此对象)

- 论文主线已改为“经典输入 → 坐标变换 → 零点定位与反射配对 → Hardy 定理传输
  → RH 缺口”：临界圆上的无限零点来自 Hardy 的已知定理，而不是由能量、频率
  或形式化定理清单独立推出。能量与频率只作为坐标诊断；圆环中的非自配对排除
  仍是 RH 本身。
