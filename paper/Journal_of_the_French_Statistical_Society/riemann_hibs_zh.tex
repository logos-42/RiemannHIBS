...
%
% 中文排版采用 xeCJK/XeLaTeX 路线，含以下针对本仓库 jsfds.cls/jsfds.sty 的
% 主文件级规避补丁（均不改动类文件本身；英文版 riemann_hibs_en.tex 若要改走
% XeLaTeX 路线需做同样处理）：
...
%   * 【\Lean 下划线安全化】Lean 定理名含 "_"（catcode 8 数学字符），在文本
%     模式会触发 "Missing $ inserted"；且定理可选标题属于 moving argument，
%     会进入 jsfds/amsthm 的展开链。\Lean 因此重定义为
%     \texttt{\detokenize{#1}}，标题内则进一步包成
%     \mbox{\texttt{\detokenize{...}}} 双保险。
%   * 【\printcontact 规避】XeTeX 下 jsfds 的 \printead/\saferef 链在首次编译
%     （引用未定义）时会触发 "Incomplete \iffalse"，故作者栏邮箱改为纯文本
%     直接排版（内容不变，仅失去 mailto 链接）。
%   * 【致谢环境补齐】jsfds 类并未定义 acknowledgement 环境（英文版的
%     \begin{acknowledgement} 同样无法编译），在 \startlocaldefs 中补齐。
%   * 中文字体使用 macOS 自带 Songti SC（备选 STSong，见下行注释）；另用
%     \xeCJKDeclareSubCJKBlock 把希腊区块划给 CJK 字体，使关键词中的裸字符
%     "ζ" 正常输出。
%
...
\xeCJKDeclareSubCJKBlock{Greek}{"0370 -> "03FF}
...
% jsfds 类未内置致谢环境（英文版同样缺失），此处按期刊风格补齐
\newenvironment{acknowledgement}
  {\par\medskip\noindent{\itshape 致谢}\ \ignorespaces}
  {\par}
\endlocaldefs
...
\begin{aug}
  ...
  \affiliation[t1]{Independent Researcher. \\ yuanjieliu65@gmail.com}
...
\begin{proposition}[主支；\,\mbox{\texttt{\detokenize{expZeta_phase_principal}}}]\label{prop:principal}
...
\mbox{\texttt{\detokenize{phase_periodic}}}, \mbox{\texttt{\detokenize{phase_fold_neg_axis}}},
\mbox{\texttt{\detokenize{phase_fold_many_sheets}}}]\label{prop:periodic}
对每个整数 $k$ 与每个可容许的点，
\[
...
\begin{theorem}[每一圈之上皆有分支；\,\mbox{\texttt{\detokenize{envelope_universal_cover_branch}}},
\mbox{\texttt{\detokenize{helix_continuation}}}]\label{thm:branches}
...
\begin{theorem}[覆盖—平面对应；\,\mbox{\texttt{\detokenize{zetaCoverIsomorphism}}}]
\label{thm:coveriso}
...
\begin{theorem}[临界线映为圆；\,\mbox{\texttt{\detokenize{criticalLine_circle}}}]
...
\begin{proposition}[\mbox{\texttt{\detokenize{log_sqrt_exp_one}}}]\label{prop:logsqrt}
$\log(\sqrt{e}) = \tfrac12$。
\end{proposition}
...
\begin{theorem}[唯一不动圆；\,\mbox{\texttt{\detokenize{envelope_inversion_fixed_circle}}}]
...
\mbox{\texttt{\detokenize{envelope_inversion_eq_conj_on_circle}}}]\label{thm:invconj}
...
\begin{theorem}[作为反演的反射；\,\mbox{\texttt{\detokenize{envelope_inversion_map}}}]
\label{thm:map}
...
\begin{theorem}[闭合机制环；\,\mbox{\texttt{\detokenize{mechanism_cycle_closed}}}]
\label{thm:cycle}
...
\begin{theorem}[远侧；\,\mbox{\texttt{\detokenize{zero_free_outside_envelope}}}]\label{thm:farside}
若 $\Re s\geq 1$ 则 $\zeta(s)\neq 0$。证明搬运欧拉乘积的正性：每个局部因子非零，
故乘积非零（参见 MathLib 的 \Lean{riemannZeta_ne_zero_of_one_le_re}）。
\end{theorem}

\begin{theorem}[负侧；\,\mbox{\texttt{\detokenize{zero_free_negative_side}}}]
\label{thm:negside}
若 $\Re s\leq 0$ 且 $s\notin\{0,-2,-4,-6,\dots\}$，则 $\zeta(s)\neq 0$；论证经
函数方程把远侧结果作反射（定理~\ref{thm:map}）。
\end{theorem}

\begin{corollary}[临界带；\,\mbox{\texttt{\detokenize{nontrivial_zero_in_critical_strip}}}]
\label{cor:strip}
每个非平凡零点满足 $0<\Re s<1$。
\end{corollary}
...
\begin{theorem}[圆环定位；\,
\mbox{\texttt{\detokenize{nontrivial_zero_in_envelope_annulus}}}]\label{thm:annulus}
若 $s$ 是非平凡零点，则
\[
  1 \;<\; \Norm{e^{s}} \;<\; e\, ,
  \qquad\text{且}\qquad
...
\mbox{\texttt{\detokenize{infinitely_many_trivial_zeros}}}]\label{thm:trivial}
...
\begin{proposition}[\mbox{\texttt{\detokenize{inversion_preserves_annulus}}}]\label{prop:preserves}
反演 $w\mapsto e/w$ 保持圆环 $1<\abs{w}<e$。
\end{proposition}

\begin{definition}[圆环非平凡零点；\,
\mbox{\texttt{\detokenize{AnnularNontrivialZero}}}]\label{def:annzero}
...
\mbox{\texttt{\detokenize{annularNontrivialZero_reflects}}},
\mbox{\texttt{\detokenize{annularNontrivialZero_iff_reflects}}},
\mbox{\texttt{\detokenize{on_circle_reflects_on_circle}}}]\label{prop:closes}
...
\mbox{\texttt{\detokenize{exists_annular_nontrivial_zero_pair}}}]\label{thm:pairs}
...
\begin{definition}[带窗；\,\mbox{\texttt{\detokenize{hiddenStripWindow}}},
\mbox{\texttt{\detokenize{hiddenLiftRectangle}}}]\label{def:window}
...
\mbox{\texttt{\detokenize{hiddenRectangleBoundaryIntegral}}},
\mbox{\texttt{\detokenize{hiddenRectangleBoundaryIntegral_eq_zero_of_differentiableOn}}}]
...
\mbox{\texttt{\detokenize{hiddenNontrivialZeroHeights}}},
\mbox{\texttt{\detokenize{HiddenZeroHeightGrowth}}}]\label{def:heightgrowth}
圆环非平凡零点被映射到它们的覆盖高度
$\abs{\Im s}$。\emph{隐零点高度增长}即是断言：该高度集合无界。
\end{definition}

\begin{theorem}[高度增长给出无限性；\,
\mbox{\texttt{\detokenize{infinite_annular_nontrivial_zeros_of_height_growth}}},
\mbox{\texttt{\detokenize{infinite_nontrivial_zeros_of_hidden_height_growth}}}]
...
\begin{proposition}[长度锁定；\,\mbox{\texttt{\detokenize{critical_leaf_norm_locked}}}]
\label{prop:lock}
...
\mbox{\texttt{\detokenize{absolute_convergence_outside_critical_leaf}}},
\mbox{\texttt{\detokenize{critical_leaf_not_absolutely_convergent}}}]\label{prop:convdiv}
绝对收敛严格成立于半径 $e$ 之外（更靠外的叶），而临界叶 $r=\sqrt{e}<e$ 不绝对收敛：
临界叶正坐在分界的非绝对收敛一侧。
\end{proposition}

\begin{proposition}[缠绕计数；\,\mbox{\texttt{\detokenize{term_winding_number}}}]
...
\mbox{\texttt{\detokenize{phaseCancelRelation}}}]\label{def:cancel}
有限整数组合 $\sum_i a_i\log(n_i+1)$ 称为\emph{抵消}，若它落在
$2\pi\mathbb{Z}$ 中：有限多个频率经整数加权后，在单位圆上回到出发角度。
\end{definition}

\begin{proposition}[\mbox{\texttt{\detokenize{phaseCancel_of_exp_one}}}]\label{prop:expone}
抵消推出 $\exp\!\bigl(i\sum_i a_i\log(n_i+1)\bigr)=1$，即单位向量之积回到 $1$。
\end{proposition}

\begin{theorem}[抵消等于整数幂恒等式；\,
\mbox{\texttt{\detokenize{phaseCancel_zero_iff_powProduct_one}}}]\label{thm:cancel}
有限相位抵消等价于如下整数乘法恒等式
\[
...
\mbox{\texttt{\detokenize{eta_eq_mul_zeta}}}, \mbox{\texttt{\detokenize{eta_term_rotating_vector}}},
\mbox{\texttt{\detokenize{eta_phase_alignment_condition}}}]\label{prop:eta}
对 $\Re s>1$，
\[
...
\begin{definition}[\mbox{\texttt{\detokenize{alignmentZero}}}]\label{def:align}
$s$ 称为\emph{对齐零点}，若其交替旋转向量和（$\eta$ 形态）等于零。
\end{definition}

\begin{theorem}[\mbox{\texttt{\detokenize{alignmentZero_iff_zeta_zero}}}]\label{thm:align}
...
\begin{theorem}[单零点原子；\,
\mbox{\texttt{\detokenize{argumentPrinciple_single_zero}}}]\label{thm:atom}
...
\mbox{\texttt{\detokenize{envelope_circle_param}}}]\label{prop:param}
圆 $\abs{z-c}=r$ 在包络相位坐标中由 $\theta\mapsto c+r e^{i\theta}$ 参数化。
\end{proposition}

\begin{theorem}[相位缠绕计零点；\,
\mbox{\texttt{\detokenize{phase_winding_equals_zero_count}}}]\label{thm:phasewind}
以对数导数的形式表述：相位沿一条围道的缠绕等于（所围零点数）乘 $2\pi i$：
每个零点对应整整一圈相位。
\end{theorem}

\begin{proposition}[\mbox{\texttt{\detokenize{zeroCount_normalized}}}]\label{prop:norm}
归一化消去常数：$(2\pi i)^{-1}\oint dz/(z-w_{0})=1$。
\end{proposition}

\begin{theorem}[幂因子绕数；\,
\mbox{\texttt{\detokenize{powFactor_logDeriv_eq}}}, \mbox{\texttt{\detokenize{winding_of_pow_factorization}}}]
\label{thm:powwind}
...
\mbox{\texttt{\detokenize{ZetaSimpleZeroCertificate}}}]\label{def:cert}
...
\begin{theorem}[证书的推论；\,
\mbox{\texttt{\detokenize{certificate_gives_zero}}},
\mbox{\texttt{\detokenize{zeta_winding_eq_two_pi_i_of_certificate}}},
\mbox{\texttt{\detokenize{annulusZeroWitness_of_zetaCertificate}}},
\mbox{\texttt{\detokenize{exists_annular_pair_of_zetaCertificate}}}]\label{thm:certchain}
...
\begin{theorem}[增长分解；\,
\mbox{\texttt{\detokenize{zeta_growth_decomposition}}}]\label{thm:decomp}
...
\begin{proposition}[\mbox{\texttt{\detokenize{gammaR_norm_decomposition}}},
\mbox{\texttt{\detokenize{gammaR_norm_in_envelope_coords}}},
\mbox{\texttt{\detokenize{zeta_norm_in_envelope_coords}}}]\label{prop:normread}
纯代数地，
\[
...
\begin{proposition}[\mbox{\texttt{\detokenize{analyticAt_riemannZeta}}}]\label{prop:analytic}
$\zeta$ 在每个 $s\neq 1$ 处解析；证明经由去心邻域的可微性再以连续性拼接而成。
\end{proposition}

\begin{theorem}[\mbox{\texttt{\detokenize{logDeriv_circle_integral_eq_zero_of_zero_free}}}]
\label{thm:zerofreeint}
若 $\zeta$ 在闭盘内既无零点也无极点，则包围圆上的对数导数积分为零。
\end{theorem}

\begin{theorem}[逆否存在性之桥；\,
\mbox{\texttt{\detokenize{exists_closedBall_zero_of_logDeriv_integral_ne_zero}}}]
...
\begin{definition}[\mbox{\texttt{\detokenize{AnnulusZeroWitness}}},
\mbox{\texttt{\detokenize{CriticalStripWitness}}}]\label{def:witness}
...
\mbox{\texttt{\detokenize{ZeroHeightSupply}}}]\label{def:supply}
\emph{高度供给}是断言：在每个高度 $H$ 之上都存在一个临界带零点。它是
$N(T)\to\infty$ 的直接弱化；它被形式化为一个显式定义，既非公理也非已证定理。
\end{definition}

\begin{theorem}[供给链；\,
\mbox{\texttt{\detokenize{hiddenZeroHeightGrowth_of_zeroHeightSupply}}},
\mbox{\texttt{\detokenize{infinite_nontrivial_zeros_of_zeroHeightSupply}}}]\label{thm:chain}
...
\mbox{\texttt{\detokenize{UnboundedZetaCertificateAssumption}}}]\label{deffamily}
\emph{无界证书族}是条件谓词（形式化为定义，而非公理）：在每个高度之上都存在一个
单零点证书实例（定义~\ref{def:cert}），其盘位于临界带之内。
\end{definition}

\begin{theorem}[族之桥；\,
\mbox{\texttt{\detokenize{zeroHeightSupply_of_certificateFamily}}},
\mbox{\texttt{\detokenize{classicalZeroCountGrowth_of_zeroHeightSupply}}},
\mbox{\texttt{\detokenize{ModulatedZetaFamily}}},
\mbox{\texttt{\detokenize{zeroHeightSupply_of_modulatedFamily}}},
\mbox{\texttt{\detokenize{infinite_nontrivial_zeros_of_modulatedFamily}}}]\label{thm:family}
下列蕴含成立：
\[
  \textup{证书族}
...
\begin{proposition}[\mbox{\texttt{\detokenize{critical_leaf_diagonal_energy_term}}}]\label{prop:diagterm}
...
\begin{theorem}[\mbox{\texttt{\detokenize{critical_leaf_diagonal_energy_diverges}}}]
\label{thm:diverge}
...
\begin{definition}[\mbox{\texttt{\detokenize{rotVecOnLine}}}, \mbox{\texttt{\detokenize{dirichletPartialSumLine}}}]
\label{def:template}
...
\mbox{\texttt{\detokenize{integral_rotating_atom}}}]\label{prop:intatom}
对 $\Delta\neq 0$，
\[
...
\mbox{\texttt{\detokenize{energy_single_term_integral}}}]\label{thm:energyint}
在长度为 $2T$ 的高度窗口上，
\[
...
\begin{theorem}[递推下界；\,
\mbox{\texttt{\detokenize{Real.gamma_ge_pow_mul_self}}}]\label{thm:gamma}
对一切 $x\geq 1$ 与自然数 $n$，
\[
...
\section{能量平衡与隐均值定理}\label{sec:balance}

三个内在组件把“能量发散”从定性收紧为定量：唯一的能量平衡半径、交叉项的截断
相消，以及由此得到的隐均值定理。

\begin{theorem}[能量平衡半径；\,\mbox{\texttt{\detokenize{energy_balance_radius}}}]
\label{thm:balance}
...
\begin{proof}
级数 $\sum_n (n+1)^{-2\sigma}$ 与 $p$-级数
$\sum_{n\geq1} n^{-2\sigma}$（平移）同敛散；MathLib 的 \Lean{summable_nat_rpow_inv}
给出收敛当且仅当 $2\sigma>1$，即 $\sigma>\tfrac12$。
\end{proof}

\begin{remark}[唯一平衡点与 $1/2$ 的身份]\label{remark:balance}
定理~\ref{thm:balance} 给出“临界线为何是 $1/2$”的\emph{能量}版本：
$\sigma=\tfrac12$ 是调和边界——其上能量超调和而收敛（预算有限），其下次调和而发散
（预算爆炸）。它与反射不动点 $\sigma=1-\sigma$（反演不动圆
$\abs{w}=\sqrt{e}$，命题~\ref{prop:logsqrt}）指向同一半径。
\textbf{平衡点给出唯一的候选半径，但其本身并不强迫零点落在那里}——那正是 RH
本身（见诚实边界 (vi)）。
\end{remark}

\begin{theorem}[交叉项极小界；\,\mbox{\texttt{\detokenize{rotating_integral_bound_min}}}]
\label{thm:crossmin}
对 $\Delta\neq 0$，
\[
...
\begin{proof}
两个界分别来自 $\abs{e^{i\Delta t}}=1$ 的平凡区间长度界，以及精确的旋转积分原子
$\int_{-T}^{T}e^{i\Delta t}dt=(e^{i\Delta T}-e^{-i\Delta T})/(i\Delta)$。
\end{proof}

\begin{remark}[截断相消：为什么交叉项是次主导的]\label{remark:cutoff}
逐点三角不等式在相邻频率处爆炸（$\Delta\to 0$）；解决方案是依赖 $t$
的截断 $X(t)=\sqrt{t/2\pi}$（近似函数方程）。交叉项界按 $X(T)$ 项计数为
$O(X(T)\log X(T))=O(\sqrt{T}\log T)$，次于对角主项 $\int_0^T \log X(t)\,dt\sim
\tfrac{T}{2}\log T$（数值上，fig13 v2：cross/main $0.267\to0.049$）。
相消机制是\emph{截断}，而非振荡干涉。
\end{remark}

\begin{theorem}[精确对角积分；\,\mbox{\texttt{\detokenize{diagonal_integral}}}]
\label{thm:diagint}
\[
...
\begin{proof}
原函数 $G(t)=\tfrac{t}{2}(\log\frac{t}{2\pi}-1)$ 的导数为
$\tfrac12\log\frac{t}{2\pi}$（对数求导加乘积法则）；由微积分基本定理取值。
\end{proof}

\begin{proposition}[隐均值定理（猜想，数值确认）]\label{prop:meanvalue}
单侧截断能量
\[
...
\]
其中 $\gamma$ 为欧拉常数（调和和
$\sum_{n< N}(n+1)^{-1}-\log(N+1)\to\gamma$，
\Lean{harmonic_log_tendsto_euler}）。数值上，
$E(T)/[T\log(T/2\pi)]\to 0.5$ 且余项$/T\to\gamma\approx 0.58$
（fig13 v2）。
\end{proposition}

\begin{remark}[两侧拼装：复现 Hardy--Littlewood]\label{remark:hl}
把单侧能量加倍（主项与辅助项各占一半）即得
\[
  \int_0^T \Abs{\zeta(\tfrac12+it)}^2 dt \;=\; T\log\frac{T}{2\pi} + (2\gamma-1)\,T + o(T) ,
\]
即完整的 Hardy--Littlewood 均值展开，它由初等积分、欧拉常数与截断次主导性拼装而成
——\textbf{不借助任何 Stirling 或 $\Gamma$ 渐近}。$\gamma$
项来自调和和的常数项（不是 $\zeta$ 的解析性质）；$o(T)$
（交叉项次主导性）的严格形式化是剩余缺口（诚实边界 (vi)）。
\end{remark}
...
\subsection*{(vi) 能量一侧的进展与两个剩余缺口}
第 11 节的三个内在组件大幅收紧了能量一侧，但仍有两个被命名的缺口保持开放。
\textbf{R1（交叉项次主导性的形式化）}：截断相消机制已获数值确认（fig13 v2：
cross/main $0.267\to0.049$），且极小界
（定理~\ref{thm:crossmin}）与精确对角积分
（定理~\ref{thm:diagint}）已在 Lean 中得证；但使交叉项界
$O(\sqrt{T}\log T)$ 成为次主导的严格双重和估计（Hardy--Littlewood 型 $L^2$
论证）尚未形式化——它恰是命题~\ref{prop:meanvalue} 中的 $o(T)$。
\textbf{R2（平衡点 $\to$ 共圆性）}：能量平衡半径
（定理~\ref{thm:balance}）与反射不动点使 $\sigma=\tfrac12$ 成为\emph{唯一候选}
半径，但“零点位于此圆上”仍是 RH 本身（$\tfrac12<\Re s<1$ 无零点）；仅凭平衡不能
强迫零点位置。隐数坐标系已完成翻译、身份、能量一侧与一一对应（零点的可数性、
孤立性与双射枚举均经 Lean 证明），把 RH 压缩为两个被命名的缺口：R1 是工程问题
（经典估计的 Lean 化），R2 是数学问题（经典上同样未解）。
